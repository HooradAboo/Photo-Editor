...
int image::getPixel(const int row, const int col, const int rgb) {
    ...
    switch (rgb) {
        case RED:
            value = data.redChannel[row * data.numColumns + col];
            break;
        case GREEN:
            value = data.greenChannel[row * data.numColumns + col];
            break;
        case BLUE:
            value = data.blueChannel[row * data.numColumns + col];
            break;
    }
    return value;
}
...
bool image::save(const char *file) {
    ...
    if (strstr(file, ".ppm") != NULL) {    /* PPM Color Image */
        flag = 0;
    } else if (strstr(file, ".pgm") != NULL) {    /* PGM Gray-level Image */
        flag = 1;
    } else {
        fprintf(stderr, "\nIncorrect File Type\n");
        return false;
    ...
    if (flag) {
        for (int i = 0; i < data.numRows; i++) {
            for (int j = 0; j < data.numColumns; j++) {
                outfile << (unsigned char) (((data.redChannel[i * data.numColumns + j]) > 255) ? 255 : (data.redChannel[
                        i * data.numColumns + j]));
            }
        }
    } else {
        for (int i = 0; i < data.numRows; i++) {
            for (int j = 0; j < data.numColumns; j++) {
                outfile << (unsigned char) (((data.redChannel[i * data.numColumns + j]) > 255) ? 255 : (data.redChannel[
                        i * data.numColumns + j]));
                outfile << (unsigned char) (((data.greenChannel[i * data.numColumns + j]) > 255) ? 255
                                                                                                 : (data.greenChannel[
                                i * data.numColumns + j]));
                outfile << (unsigned char) (((data.blueChannel[i * data.numColumns + j]) > 255) ? 255
                                                                                                : (data.blueChannel[
                                i * data.numColumns + j]));
            }
        }
...
bool image::read(char *filename) {
    ...
    if (strstr(filename, ".ppm") != NULL) {    /* PPM Color Image */
        flag = 0;
    } else if (strstr(filename, ".pgm") != NULL) {    /* PGM Gray-level Image */
        flag = 1;
    } else {
        fprintf(stderr, "\nIncorrect File Type\n");
        return false;
